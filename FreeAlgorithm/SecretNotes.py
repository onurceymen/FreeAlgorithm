...
def save_and_encrypt_notes():
    ...
    if len(title) == 0 or len(message) == 0 or len(master_secret) == 0:
        messagebox.showinfo(title="Error", message="Please fill in all information.")
    else:
...
def decrypt_notes():
    ...
    if len(message_encrypted) == 0 or len(master_secret) == 0:
        messagebox.showinfo(title="Error!", message="Please enter the decryption key.")
    else:
        try:
            ...
            messagebox.showinfo(title="Error", message="Invalid decryption key or encrypted data.")
...
input_info_label = Label(text="Enter Secret Information", font=FONT)
